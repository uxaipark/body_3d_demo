...
type Page='home'|'body'|'sleep';
// Document navigation gives every simulator its own WebGL/worker lifecycle.
export function SimulatorNav({active}:{active:Page}){return <L as="nav" className={styles.nav} aria-label="시뮬레이터 메뉴">{([['home','/','시뮬레이터'],['body','/simulators/body','전신 트윈'],['sleep','/simulators/sleep','수면무호흡']] as const).map(([key,href,label])=><L as="a" key={key} href={href} aria-current={active===key?'page':undefined}>{label}</L>)}</L>}
