...
export class SleepScene{
 ...
 constructor(host:HTMLDivElement,progress:(n:number)=>void,private onPosition:(p:PatchPosition,geometry:{electrodes:number[][];up:number[]})=>void=()=>{}){
  ...
  for(let i=0;i<3;i++){const pad=new T.Mesh(new T.CylinderGeometry(i===1?.011:.006,i===1?.011:.006,.0015,32),new T.MeshStandardMaterial({color:i===1?'#f8c86a':'#a3b0b7',metalness:.45,roughness:.38}));pad.name=i===1?'CAP sensing pad':i===0?'ECG E- contact':'ECG E+ contact';pad.rotation.x=Math.PI/2;pad.position.set((i-1)*.028,0,.0025);this.patch.add(pad);this.attachments.push({mesh:pad,x:(i-1)*.028,y:0,height:.0025,anchors:[]});}
