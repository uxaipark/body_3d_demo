...
export class PatientScene{
 avatar:Avatar;posture=new PostureController();ready=false;overheadView=false;ring:T.Group;previousHeight:number|null=null;contact=true;
 ...
 const overhead=c.arm==='raised'&&c.posture!=='walking'&&c.posture!=='lying';
 if(overhead!==this.overheadView){this.overheadView=overhead;const v=this.avatar.view;v.focus(c.posture==='lying'?'bed':'body');if(overhead){v.controls.target.set(0,1.10,0);v.camera.position.set(.7,1.4,4.5);v.controls.update();}this.avatar.lastOrbit='';}
