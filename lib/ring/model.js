...
export function ringSample(t,{hr=64,rr=14,spo2=97,motion=false,contact=true,heightCm=0,cycle=false,sbp=120,dbp=80,heightVelocity=0}={}){
...
export function ringCsv(samples){return 'time_s,red_normalized,ir_normalized,acc_x_g,acc_y_g,acc_z_g,simulated_hr_bpm,simulated_spo2_percent,quality_percent,height_above_heart_cm,local_sbp_mmhg,local_dbp_mmhg,hydrostatic_offset_mmhg\n'+samples.map(s=>[s.t.toFixed(3),s.red,s.ir,s.ax,s.ay,s.az,s.hr??'',s.spo2??'',s.quality,s.heightCm,s.sbp,s.dbp,s.offset].join(',')).join('\n');}
